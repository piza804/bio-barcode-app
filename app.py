...
if menu == "バーコード登録":
    ...
    window.addEventListener('message', (event) => {
      if (event.data.type === 'barcode') {
        const input = window.parent.document.querySelector('input[id*="barcode番号"]');
        if(input){
    ...
    if barcode_data:
        ...
        if docs:
            ...
            if st.button("数量 +1"):
                ...
                db.collection("reagents").document(doc_id).update({
                    "qty": new_qty,
            ...
            st.warning("新規バーコードです。以下を入力して登録してください。")
