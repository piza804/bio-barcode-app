...
if menu == "バーコード登録":
    ...
    """
    st.components.v1.html(quagga_html, height=500, scrolling=False)

    # JS → Streamlit にバーコードを送信
    st.markdown("""
    <script>
    ...
    if barcode_data:
        ...
        if docs:  # 既存試薬
            ...
            name = st.text_input("試薬名", key="name_input")
            qty = st.number_input("数量", 1, 100, 1, key="qty_input")
            exp = st.date_input("有効期限", key="exp_input")
            if st.button("登録"):
